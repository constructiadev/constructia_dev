/*
  # Nueva Estructura Multi-Tenant para ConstructIA

  1. Nuevas Tablas
    - `tenants` - Inquilinos/clientes principales
    - `users` - Usuarios con roles específicos por tenant
    - `empresas` - Empresas dentro de cada tenant
    - `obras` - Obras/proyectos de construcción
    - `proveedores` - Proveedores homologados
    - `trabajadores` - Personal de los proveedores
    - `maquinaria` - Equipos y maquinaria
    - `documentos` - Documentos con categorización específica
    - `tareas` - Sistema de tareas y flujo de trabajo
    - `requisitos_plataforma` - Requisitos por plataforma externa
    - `mapping_templates` - Plantillas de mapeo para integraciones
    - `adaptadores` - Conectores a plataformas externas
    - `jobs_integracion` - Cola de trabajos de integración
    ...
    - `mandatos_sepa` - Mandatos SEPA firmados
    - `manual_upload_queue` - Cola de subida manual

  2. Seguridad
    - RLS habilitado en todas las tablas
    - Políticas basadas en tenant_id y roles
    - Separación completa entre tenants

  3. Índices
    - Índices únicos por tenant donde corresponde
...
-- Tabla Documentos (nueva estructura)
...
-- Tabla Suscripciones (nueva estructura)
...
-- Tabla Auditoría (nueva estructura)
...
-- Políticas RLS para SuperAdmin (acceso global)
CREATE POLICY "SuperAdmin can access all tenants" ON tenants
...
-- Políticas RLS para todas las demás tablas (acceso por tenant)
